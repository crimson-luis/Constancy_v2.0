...
def write_file(loc, json_str):
    """Opens or creates the file and writes the given encrypted (cryptography.fernet.Fernet) json string to it."""
...
def get_info(loc):
    """Returns a tuple with a dictionary of the owner information and the id of the last item (or the number of items)
    """
...
def get_users():
    """Returns a tuple with a dictionary of all users and their id's and a variable that refers to the number of
    files/users."""
    _files = []
    users_ids = dict()
    id_count = 0
...
def create_item(dic, description, value, date, installments, _type, sub_items=None):
    """Returns a dictionary with all user information including the new item that it creates. If sub_items is given
    then create_sub_item function is called."""
...
def create_user(name, password, email, loc):
    """Creates a dictionary with all information given, calls create_item function to create the first user item and
     saves the user file (name.encrypted)."""
    dic = {
        "user": {
            "id": get_users()[1] + 1,
            "name": name,
